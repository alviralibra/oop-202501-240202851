...
Contoh: *Mengintegrasikan seluruh materi Bab 1–13 ke dalam satu aplikasi yang utuh.*)
...
Contoh:  
1. OOP (Object-Oriented Programming) memungkinkan aplikasi dibangun secara modular, reusable, dan terstruktur.
2. DAO (Data Access Object) memisahkan logika akses data dari logika bisnis aplikasi.
3.MVC Architecture memisahkan View, Controller, dan Model agar aplikasi mudah dikembangkan.)
...
A. Manajemen Produk
Menampilkan daftar produk dari database menggunakan TableView<Product>.
Menambah produk ke database melalui ProductService → ProductDAO.
Menghapus produk dari database melalui tombol GUI.
Validasi input dilakukan dengan custom exception (mis. kode kosong, harga tidak valid).

B. Keranjang Belanja
Keranjang disimpan menggunakan Collections (List<CartItem>).
...
C. Pattern dan Testing
Design Pattern:
Singleton digunakan pada koneksi database (JDBC Connection).
Unit Test:
...
![Screenshot hasil](screenshots/main%20program.png)
![Screenshot hasil](screenshots/test.png)
)
---
...
- Menampilkan daftar produk dari database menggunakan TableView<Product>.
-Menambah produk ke database melalui ProductService → ProductDAO. 
-Kendala: Error sinkronisasi TableView setelah delete data
Solusi: Memanggil ulang method loadData() setelah operasi delete.
)
---

## Kesimpulan
( kesimpulan dari praktikum minggu ini: *Praktikum Bab 14 berhasil mengintegrasikan seluruh materi dari Bab 1 hingga Bab 13 ke dalam satu aplikasi Agri-POS yang utuh. Aplikasi telah menerapkan prinsip OOP, arsitektur MVC, DAO, Collections, exception handling, design pattern, unit testing, serta GUI JavaFX secara konsisten dan terstruktur. Aplikasi ini siap dikembangkan lebih lanjut sebagai dasar tugas akhir atau UAS.*)
